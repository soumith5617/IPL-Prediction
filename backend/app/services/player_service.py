...
class PlayerService:
    ...
    def get_players(
        ...
        if sort_by == "runs":
            ...
            query = query.filter(Player.total_runs >= 200).order_by(desc(Player.strike_rate))
        elif sort_by == "average":
            query = query.filter(Player.total_runs >= 200).order_by(desc(Player.batting_average))
    ...
    def get_player_by_id(db: Session, player_id: int) -> Optional[Dict[str, Any]]:
        ...
        # Build 6-axis Radar metric for player performance
        # Normalized against benchmark caps
        ...
        summary = f"{player.name} is a {player.country} cricketer ({player.batting_hand} bat, {player.bowling_skill}). Across {player.matches} IPL matches, accumulated {player.total_runs} runs (HS: {player.highest_score}, SR: {player.strike_rate}) and claimed {player.wickets} wickets (Econ: {player.economy})."
